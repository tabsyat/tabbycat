...
OPTIONS_TO_CONFIG_MAPPING = {
    ...
    "pullup_eligible_pct"   : "draw_rules__draw_pullup_eligible_pct",
...
class PowerPairedDrawManager(BaseDrawManager):
    ...
    def get_relevant_options(self):
        ...
        if self.teams_in_debate == 2:
            options.extend([
                ...
                "pullup_restriction", "pullup_eligible_pct", "side_allocations",
    ...
    def get_teams(self) -> Tuple[List['Team'], List['Team']]:
        ...
        if pullup_restriction == "eligible_pct":
            extra_metrics.add("npullups")
            extra_metrics.add("speaks_sum")
        ...
        for standing in standings:
            ...
            if pullup_restriction == "eligible_pct":
                team.npullups = standing.metrics.get('npullups', 0)
                team.speaks_sum = standing.metrics.get('speaks_sum', 0)
            if self.round.tournament.pref("draw_odd_bracket") == 'pullup_lowest_ds_rank_npulls':
